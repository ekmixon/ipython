...
class IPCompleter(Completer):
    ...
    def dict_key_matches(self, text:str) -> List[str]:
        ...
        if key_start > text_start and closing_quote:
            ...
            if continuation.startswith(closing_quote):
                ...
                suf += closing_quote
        if bracket_idx > text_start and not continuation.startswith(']'):
            suf += ']'
    ...
    def dispatch_custom_completer(self, text):
        ...
        try_magic = (
            []
            if cmd.startswith(self.magic_escape)
            else self.custom_completers.s_matches(self.magic_escape + cmd)
        )
    ...
    def _complete(self, *, cursor_line, cursor_pos, line_buffer=None, text=None,
        ...
        if self.backslash_combining_completions:
            ...
            base_text = line_buffer[:cursor_pos] if line_buffer else text
    ...
    def fwd_unicode_match(self, text:str) -> Tuple[str, Sequence[str]]:
        ...
        return (
            (s, candidates)
            if (candidates := [x for x in self.unicode_names if x.startswith(s)])
            else ('', ())
        )
